...
typedef struct
{
	uint8_t mods; // Bitfield contatning keyboard modifiers
	uint8_t keys[KEYS_PER_REPORT]; //Up to 6 keys per report
} keyboard_report;
#endif
void usb_keyboard_set_report(keyboard_report*);
...
#define USB_FREEZE() (USBCON = ((1<<USBE)|(1<<FRZCLK)|(1<<OTGPADE)|(1<<VBUSTE)))
